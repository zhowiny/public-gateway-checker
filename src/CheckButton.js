...
class CheckButton extends UiComponent {
    constructor(parent) {
        super(parent);
        this.parent = parent;
        const CheckButtonElement = document.getElementById('checker.button');
        if (CheckButtonElement == null) {
            throw new Error('Could not find element with Id "checker.button"');
        }
        this.tag = Tag.fromElement(CheckButtonElement);
